...
$pvalc=11; # col index for pvalue
$chrc=1; # col index for chr
$posc=2; # col index for pos-hg19 
$weic=9; # col index for effect size 
$a1c=4; # col index for allele 1, effect allele
$a2c=5;
...
$weightfile = "/dcs05/lieber/hanlab/shan/prs/stem_cell_core/weights_no_lieber/pgc3_scz_maf0.01_info0.9_hg38";
if($weightfile =~ /\.gz$/){
...
while(<IN>){
	...
	if($tag_count{$bar2} == 3){
		...
		$weight{$bar} = log($tokens[$weic]); # beta
		$a1{$bar} = $tokens[$a1c];
