...
phe[, Dx2 := fifelse(Dx %in% c("Control"), "Control",
              fifelse(Dx %in% c("MDD"), "MDD", NA_character_))]
phe <- phe[!is.na(Dx2)]
...
dat[, Group := factor(Group, levels = c("Control","MDD"))]

## ---- function to draw paginated boxplots with p-values ----
plot_prs_by_race <- function(df, race) {
  dfr <- df[Race == race]
  
  # Compute p-values + per-threshold y-range for proper label placement
  pvals <- dfr[, {
   ...
   pv <- if (length(unique(na.omit(Group))) < 2L) {
     NA_real_
   } else {
     tryCatch(t.test(score ~ Group)$p.value, error = function(e) NA_real_)
   }
   
   .(p = pv, ymin = ymin, ymax = ymax, bump = bump)
  }, by = threshold_label]
  ...
  pdf(file = "mdd_control_sczPRS_boxplot.pdf", width = 12, height = 12)
  for (pg in seq_len(n_pages)) {
    g <- ggplot(dfr, aes(x = Group, y = score, fill = Group)) +
      ...
      labs(title = "SCZ PRS",
           x = NULL, y = "PRS") +
      scale_fill_manual(values = c("Control" = "#4C78A8", "MDD" = "#F58518")) +
